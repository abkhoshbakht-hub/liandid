...
const helpData: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'news-placement', title: 'جایگاه نمایش خبر', icon: '📍', content: 'در فرم خبر، بخش "جایگاه نمایش در سایت" مشخص می‌کند خبر دقیقاً کجای سایت نمایش داده شود: آخرین اخبار (پیش‌فرض)، هیرو یا اسلایدر بالای سایت، تحلیل خبر، سنجاق شده یا بالای لیست. خبرهای «هیرو» حتماً در باکس‌های هیرو می‌نشینند (جدیدترین اول، حداکثر ۳ تا) و در آخرین اخبار تکرار نمی‌شوند. اگر جایگاهی انتخاب نکنید، خبر در آخرین اخبار نمایش داده می‌شود.' },
      { id: 'instant-publish', title: 'نمایش فوری خبر در سایت', icon: '⚡', content: 'بعد از انتشار یا تایید هر خبر، صفحه اصلی و آرشیو خودکار و فوری به‌روز می‌شوند و نیازی به انتظار نیست. اگر تغییری را ندیدید، یک بار صفحه را رفرش کنید.' },
  ...
  },
  {
    ...
    icon: '🖼️',
    items: [
      { id: 'image-settings', title: 'تنظیمات عکس', icon: '⚙️', content: 'از سایدبار > مدیریت > تنظیم عکس. حداکثر حجم عکس (۱ تا ۱۰ مگابایت)، کیفیت فشرده‌سازی (۱۰ تا ۱۰۰ درصد)، حداکثر ابعاد (۸۰۰، ۱۲۸۰ یا ۱۹۲۰ پیکسل)، قاب برش پیش‌فرض (۱۶:۹، ۴:۳، ۱:۱ یا آزاد) و فرمت تبدیل (JPEG، WebP، PNG یا اصلی) را تنظیم و ذخیره کنید. روی همه عکس‌های خبری اعمال می‌شود.' },
      { id: 'image-crop', title: 'برش و تنظیم عکس خبر', icon: '✂️', content: 'در فرم خبر > عکس شاخص > انتخاب عکس. بعد از انتخاب عکس، ابزار تنظیم باز می‌شود: عکس را با موس یا انگشت بکشید تا در قاب قرار گیرد، با دکمه‌های کم (−) و زیاد (+) یا نوار لغزنده اندازه را تنظیم کنید، قاب (۱۶:۹، ۴:۳، ۱:۱ یا آزاد) را عوض کنید و «تایید و آپلود» را بزنید.' },
      { id: 'image-format', title: 'تبدیل فرمت عکس', icon: '🔄', content: 'هر عکسی با هر فرمتی آپلود شود، خودکار به فرمتی که در تنظیم عکس انتخاب کرده‌اید تبدیل می‌شود (پیش‌فرض WebP مثل سایت‌های خبری). کیفیت خودکار کم می‌شود تا حجم هر عکس زیر ۲۰۰ کیلوبایت بماند و سایت سریع بماند.' },
      { id: 'newspaper-tab', title: 'تب صفحه اول روزنامه‌ها', icon: '📰', content: 'در باکس «اخبار لحظه‌ای خبرگزاری‌ها» تب «صفحه اول روزنامه‌های کشور» خبرهای روزنامه‌ها (همشهری، اطلاعات، رسالت، اعتماد، دنیای اقتصاد و دو ورزشی) را بدون نیاز به تایید نشان می‌دهد. برای تصویر جلدها: بالای همین تب دکمه «مشاهده تصویر صفحه اول روزنامه‌ها» هست که به صفحه کیوسک می‌رود.' },
      { id: 'frontpages', title: 'ثبت جلد روزنامه‌ها', icon: '🗞️', content: 'از سایدبار > مدیریت > جلد روزنامه‌ها. هر صبح: تاریخ (خودکار امروز است)، نام روزنامه، دکمه «انتخاب عکس جلد»؛ عکس را کادر کنید و تایید بزنید. جلدها در صفحه «کیوسک» سایت نمایش داده می‌شوند. جلدهای قدیمی‌تر از ۷ روز خودکار پاک می‌شوند.' },
      { id: 'shortlinks', title: 'کوتاه‌کننده لینک', icon: '🔗', content: 'از سایدبار > مدیریت > کوتاه‌کننده لینک. لینک بلند خبر را بچسبانید و «کوتاه کن» را بزنید؛ یک لینک کوتاه مثل liandid.ir/s/a1b2c3 می‌گیرید که با یک کلیک کپی می‌شود و تعداد بازدیدش هم نمایش داده می‌شود. راحت‌تر: در لیست اخبار، کنار هر خبر دکمه «لینک کوتاه» هست که مستقیم لینک کوتاه همان خبر را کپی می‌کند.' },
      { id: 'slot-image', title: 'عکس باکس هیرو کجاست؟', icon: '🖼️', content: 'دقت کنید عکس هر باکس صفحه اصلی (مثل هیروی بزرگ) با عکس خبر فرق دارد و در دو جای جدا ذخیره می‌شود. اگر عکس باکس را در ویرایش خبر نمی‌بینید، طبیعی است. حالت اول: باکس از نوع «خبر اختصاصی» است؛ عکسش را از «مدیریت باکس‌های صفحه اصلی» پیدا کنید، روی همان باکس دکمه «انتخاب خبر» را بزنید، وارد تب «نوشتن خبر اختصاصی» شوید؛ عکس فعلی همان‌جا نمایش داده می‌شود و با دکمه آپلود یا لینک جدید می‌توانید عوضش کنید و ذخیره بزنید. حالت دوم: باکس از «آرشیو اخبار» انتخاب شده؛ در این صورت عکس باکس همان عکس شاخص همان خبر آرشیو است و باید همان خبر را ویرایش کنید. برای فهمیدن نوع باکس، در «مدیریت باکس‌های صفحه اصلی» روی هر باکس نوشته شده: «ویرایش» یعنی اختصاصی، «خبرگزاری» یعنی آرشیوی.' },
    ]
  },
  {
