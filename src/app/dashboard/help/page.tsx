...
const helpData: HelpSection[] = [
  ...
  },
  {
    ...
    icon: '🏠',
    items: [
      { id: 'hero', title: 'هیرو (بنر اصلی)', icon: '🖼️', content: 'هر باکس هیرو را با دکمه "انتخاب خبر" مدیریت کنید. دو گزینه دارید: ۱) "انتخاب از آرشیو اخبار" برای انتخاب خبر موجود ۲) "نوشتن خبر اختصاصی" برای نوشتن محتوای دستی. خبر اختصاصی خودکار آرشیو می‌شود.' },
      ...
      { id: 'latest-order', title: 'ترتیب و آرشیو آخرین اخبار', icon: '📑', content: 'خبرهای این باکس همیشه از جدید به قدیم مرتب می‌شوند (مقالات سایت و اخبار تاییدشده خبرگزاری‌ها با هم). حداکثر ۸ خبر نمایش داده می‌شود و با دکمه "مشاهده همه اخبار" پایین باکس، بازدیدکننده به صفحه آرشیو کامل می‌رسد.' },
      { id: 'latest-approve', title: 'تایید باکس‌های صفحه اصلی', icon: '✅', content: 'هر باکس صفحه اصلی وضعیت دارد: "تایید شده" (سبز) یا "در انتظار تایید" (نارنجی). فقط باکس‌هایی که خبر انتخاب شده دارند دکمه "تایید" یا "لغو تایید" نشان می‌دهند.' },
      { id: 'english-titles', title: 'حذف خودکار متن انگلیسی عناوین', icon: '🔤', content: 'کلمات و ترکیب‌های انگلیسی عناوین اخبار خبرگزاری‌ها (مثل COVID19 و G20) به‌صورت خودکار حذف می‌شوند. عناوین قدیمی هم در هر بار بروزرسانی اخبار خودکار تمیز می‌شوند. فقط عنوان تغییر می‌کند و هیچ خبری حذف نمی‌شود.' },
      ...
      { id: 'agency-news-pages', title: 'صفحه‌بندی اخبار خبرگزاری‌ها', icon: '📄', content: 'از سایدبار > صفحه اصلی > اخبار لحظه‌ای (یا بخش اخبار خبرگزاری‌ها). همه خبرها هر صفحه ۲۰ تا نمایش داده می‌شوند. با شماره صفحات پایین لیست (۱، ۲، ۳...) بین صفحات جابه‌جا شوید تا همه خبرهای بررسی‌نشده را ببینید و تایید یا رد کنید.' },
      { id: 'agency-news-approve', title: 'تایید اخبار خبرگزاری‌ها', icon: '✅', content: 'خبرهای جدید خبرگزاری‌ها با وضعیت "در انتظار تایید" می‌آیند و در سایت نمایش داده نمی‌شوند. فقط با دکمه "تایید" شما منتشر می‌شوند. با دکمه "رد" مخفی می‌مانند. هیچ خبری بدون تایید شما روی سایت نمی‌رود.' },
    ]
  },
  {
    title: 'مدیریت اخبار',
    icon: '📰',
    items: [
      { id: 'create-news', title: 'ایجاد خبر جدید', icon: '✏️', content: '۱) از سایدبار > اخبار > اخبار سایت بروید ۲) دکمه "خبر جدید" را بزنید ۳) عنوان، محتوا، دسته‌بندی و تصویر شاخص را پر کنید ۴) جایگاه نمایش خبر را انتخاب کنید ۵) وضعیت را روی "منتشر شده" بگذارید ۶) "ذخیره" را بزنید.' },
      { id: 'quick-new', title: 'منوی اضافه کردن خبر جدید', icon: '➕', content: 'از بالای سایدبار > اضافه کردن خبر جدید > خبر جدید. با یک کلیک مستقیم فرم خبر جدید باز می‌شود و می‌توانید خبر را بنویسید و ذخیره کنید.' },
      { id: 'news-placement', title: 'جایگاه نمایش خبر', icon: '📍', content: 'در فرم خبر، بخش "جایگاه نمایش در سایت" مشخص می‌کند خبر دقیقاً کجای سایت نمایش داده شود: آخرین اخبار (پیش‌فرض)، هیرو یا اسلایدر بالای سایت، تحلیل خبر، سنجاق شده یا بالای لیست. اگر جایگاهی انتخاب نکنید، خبر در آخرین اخبار نمایش داده می‌شود.' },
      { id: 'news-archive-page', title: 'آرشیو اخبار سایت', icon: '📚', content: 'صفحه آرشیو اخبار در آدرس /archive قرار دارد و از فوتر سایت با عنوان "آرشیو اخبار" قابل دسترسی است. در این صفحه همه اخبار منتشر شده با امکان جستجو، فیلتر دسته‌بندی و صفحه‌بندی نمایش داده می‌شوند.' },
      { id: 'news-archive-menu', title: 'منوی آرشیو در داشبورد', icon: '🗂️', content: 'از سایدبار > اخبار > آرشیو اخبار می‌توانید مستقیم خبرهای آرشیو شده (وضعیت آرشیو) را ببینید و مدیریت کنید. برای بازگرداندن یک خبر آرشیو شده، آن را ویرایش کنید و وضعیت را روی "منتشر شده" بگذارید.' },
      { id: 'auto-archive', title: 'آرشیو خودکار خبرهای قدیمی', icon: '⏳', content: 'خبرهایی که بیشتر از یک هفته از انتشارشان گذشته باشد، در بالای صفحه اخبار سایت با بنر "آماده آرشیو" اعلام می‌شوند. با دکمه "نمایش خبرهای آماده آرشیو" فقط همان‌ها را ببینید و برای هر خبر با دکمه "آرشیو" و تایید خودتان آن را آرشیو کنید. هیچ خبری بدون تایید شما آرشیو نمی‌شود.' },
